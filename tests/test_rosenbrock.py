...
class RosenbrockTest(unittest.TestCase):
    ...
    def test_rosenbrock_with_ineq(self):
        

        
        N = 2  # number of well parameters
        M = 10 # number of ensemble realizations
        lb = [-0.5, -1.0]  # lower bound
        ...
        def ineq1(x):
            if x.ndim == 1:
                return np.array([x[0] + x[1] - 1 for _ in range(M)])
            else:
                return x[:,0] + x[:,1] - 1
            
        def ineq2(x):
            if x.ndim == 1:
                return np.array([x[0]**2 + x[1] for _ in range(M)])
            else:
                return x[:,0]**2 + x[:,1]
        
        def ineq3(x):
            if x.ndim == 1:
                return np.array([x[0]**2 + x[1]**2 - 1 for _ in range(M)])
            else:
                return x[:,0]**2 + x[:,1]**2 - 1
                    
        # initialize u
        # u0 = np.random.rand(N) # either mean value or current best iterate value
        u0 = [0.0, 0.0]  # Initial iterate value
